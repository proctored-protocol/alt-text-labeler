...
def get_viewer_access_jwt(*, force_refresh: bool = False) -> str:
    ...
    if not token:
        raise RuntimeError(
            "TEST_VIEWER_HANDLE and TEST_VIEWER_APP_PASSWORD must be set in .env for subscriber verification"
        )
    return token
...
def is_expired_token_error(exc: HTTPJSONError) -> bool:
    ...
    if exc.code != 400:
        return False
    return isinstance(exc.payload, dict) and exc.payload.get("error") == "ExpiredToken"
...
def success(snapshot: VerificationSnapshot, *, require_forced: bool, require_subscriber: bool) -> bool:
...
def print_summary(
    *,
    ...
    print()
    print("=== monitor post label visibility summary ===")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Monitor one post over time until a label becomes externally visible."
    )
    parser.add_argument(
        "--post-url",
        required=True,
    ...
    parser.add_argument(
        "--label-value",
        required=True,
        help="Label value to look for, e.g. missing-alt-text or partial-alt-text",
    )
    parser.add_argument(
        "--monitor-timeout-seconds",
        type=int,
        default=1800,
        help="Total monitoring duration before timing out.",
    )
    parser.add_argument(
        "--interval-seconds",
        type=int,
